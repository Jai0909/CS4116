...
$sql = "SELECT *  FROM User WHERE USER_ID = $user_id";
$sql1 = "SELECT *  FROM Friend WHERE (USER_ID_1 = '$pid' AND USER_ID_2 = '$user_id') || (USER_ID_2 = '$pid' AND USER_ID_1 = '$user_id')";
...
$resultnum = mysqli_num_rows($result);
if($resultnum1 > 0) {?>
    <a href="delfriend.php?user_id=<?php echo $user_data['USER_ID']; ?>" class="btn btn-success">Delete Friend</a>
    <?php
}
else{?>
    <a href="addfriend.php?user_id=<?php echo $user_data['USER_ID']; ?>" class="btn btn-success">Add Friend</a>
    <?php
}
$conn->close();
?>
<a href="report.php?user_id=<?php echo $user_data['USER_ID']; ?>" class="btn btn-danger">Report</a>
</div>
...
while ($row = mysqli_fetch_assoc($result)) {
    // loop through columns and check column value
    ?> <a href="viewOtherProfileOrg.php?user_id=<?php echo $row['ORG_ID']; ?>" class="btn btn-success">View Employer</a><p class="percentage"><br> <p class="percentage"><?php
    foreach ($columns as $column) {
